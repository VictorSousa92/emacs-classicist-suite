;;; classicist-books.el --- open a work at one of its books -*- lexical-binding: t; -*-
...
;; THE SUITE FEATURE LIST, declared and not required: this file stands
;; alone, and absent the suite the guard falls back to opening the book.
...
(defgroup classicist-books nil
  "Opening a work at one of its books."
  :group 'diogenes
  :prefix "classicist-books-")

(defcustom classicist-books-cache-file
  (expand-file-name "classicist-books.eld" user-emacs-directory)
...
(defconst classicist-books--greek
...
(defconst classicist-books--roman
...
(defun classicist-books--as-number (said)
...
matched against the titles themselves."
  (let ((said (string-trim said)))
    (cond
     ...
     ((cdr (assoc (downcase said) classicist-books--roman)))
     ((and (= (length said) 1)
           (cdr (assq (upcase (aref said 0)) classicist-books--greek))))
     (t nil))))

(defun classicist-books--greek-letter (title)
...
`ΤΩΝ ΜΕΤΑ ΤΑ ΦΥΣΙΚΑ Α\\=', and in either the book is Alpha."
  (let ((found nil))
    (seq-doseq (char title)
      (when (and (not found) (assq char classicist-books--greek))
    ...
    (let* ((words (split-string title "[ \t\n]+" t))
      ...
      (and last (= (length last) 1)
           (assq (aref last 0) classicist-books--greek)
           (aref last 0)))))
...
(defcustom classicist-books-declared
...
(defcustom classicist-books-read-the-text '("0086")
...
For anyone else the books must be in `classicist-books-declared\=', and a work
...
(defvar classicist-books--known nil
...
(defun classicist-books--load ()
  "Read what was remembered, once."
  (when (and classicist-books-cache-file
             (null classicist-books--known)
             (file-readable-p classicist-books-cache-file))
    (setq classicist-books--known
          (ignore-errors
            (with-temp-buffer
              (insert-file-contents classicist-books-cache-file)
              (goto-char (point-min))
              (read (current-buffer)))))))

(defun classicist-books--save ()
  "Remember what has been found."
  (when classicist-books-cache-file
    (ignore-errors
      (with-temp-file classicist-books-cache-file
        (insert ";; The books of each work, as its text names them.\n")
        (insert ";; Written by classicist-books.el; delete it to have them\n")
        (insert ";; found again.\n")
        (prin1 classicist-books--known (current-buffer))
        (insert "\n")))))

(defun classicist-books--scan-buffer ()
...
be looking at the beginning of Theta, not at a particular word of it."
  (let ((found nil))
    ...
    (classicist-books--own-work (nreverse found))))

(defun classicist-books--page (citation)
...
(defun classicist-books--own-work (books)
...
being ordinary enough."
  (let ((out nil)
    ...
    (catch 'done
      (dolist (book books)
        (let ((page (classicist-books--page (cadr book))))
...
THEN is called when it has finished."
  ...
  (let* ((key (list corpus author work))
         ...
         (declared (cdr (assoc key classicist-books-declared)))
         (known (or declared (cdr (assoc key classicist-books--known)))))
    (if known
        (funcall then known)
      (unless (or (eq classicist-books-read-the-text t)
                  (member author classicist-books-read-the-text))
        (user-error
         (concat "The books of %s %s/%s are not declared, and reading the "
                 "text to find them is only done for %s -- see "
                 "classicist-books-declared and classicist-books-read-the-text")
         corpus author work
         (if classicist-books-read-the-text
             (mapconcat #'identity classicist-books-read-the-text ", ")
           "nobody")))
                ...
                (with-current-buffer buffer
                  (save-excursion
                    ...
                    (let ((n 0))
                      ...
                      n)))
                (buffer-name buffer)))
             (push (cons key books) classicist-books--known)
             (classicist-books--save)
             (message "%d books found" (length books))
             (funcall then books))))))))

(defcustom classicist-books-tries 600
...
(defun classicist-books--when-done (buffer then &optional tries)
...
looks a tenth of a second apart with no new text is a dump that has finished."
  (let ((tries (or tries classicist-books-tries))
     ...
     (lambda ()
       (cond
        ...
        ((let ((process (get-buffer-process buffer)))
         ...
         (classicist-books--when-done buffer then (1- tries)))
        ((and (= (buffer-size buffer) size) (> size 0))
         (funcall then))
        (t (classicist-books--when-done buffer then (1- tries))))))))
...
(defun classicist-books--choose (books said)
...
A word matches a title containing it, so `ΕΛΑΤΤΟΝ\\=' finds Alpha Minor."
  (let* ((said (string-trim said))
         ...
         (by-name
          (and letter
               (seq-find (lambda (book)
                           (eq (classicist-books--greek-letter (car book))
                               letter))
                         books)))
         (by-word
         ...
         (number (and (not by-name) (not by-word)
                      (classicist-books--as-number said))))
    (or by-name by-word
        (and number (> number 0) (<= number (length books))
             (nth (1- number) books)))))

(defun classicist-books--label (book n)
  "BOOK, the Nth, as the completion shows it."
  (let ((letter (classicist-books--greek-letter (car book))))
    (format "%-28s %s%s"
            (car book)
...
are remembered afterwards in `classicist-books-cache-file\\='."
  (interactive)
  ;; ASLEEP UNLESS ASKED FOR, as every feature here is.
  (unless (or (not (fboundp 'classicist-feature-p))
              (classicist-feature-p 'books))
    (user-error
     "Opening a work at a book is not among classicist-features"))
     ...
     (lambda (books)
       (let* ((labels
               (let ((n 0))
                 (mapcar (lambda (book)
                           (setq n (1+ n))
                           (cons (classicist-books--label book n) book))
                         books)))
              ...
              ;; book.
              ;;
              ;; `classicist-books--choose' still answers for SAID, a caller
              ;; naming a book outright rather than picking one from the list.
              (book (or (cdr (assoc choice labels))
                        (classicist-books--choose books choice))))
         (unless book
